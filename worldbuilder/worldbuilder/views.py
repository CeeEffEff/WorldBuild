...
from worldbuilder.dash.map_app import set_figure_map
...
def request(request: HttpRequest):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            params.use_vectorstore = {
                VECTORSTORE: form.cleaned_data["use_timeout_content"]
            }
            context_items = []
            try:
            ...
            return render(
                request, "results.html", {"data": data, "knn_results": context_items}
            )
    else:
        form = RequestForm()
...
def dash_map(request: HttpRequest):
    ...
    context = {"height_ratio": min(image_ratio, ratio)}
...
def list_points_of_interest(_: HttpRequest):
    data = serializers.serialize(
        "json", models.PointOfInterest.objects.all().order_by("name")
    )
    return HttpResponse(data, content_type="application/json")
