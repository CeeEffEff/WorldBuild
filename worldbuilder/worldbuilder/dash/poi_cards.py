...
def new_poi_card(x, y, map_id):
    ...
    return dbc.Card(
        [
            card_header(),
            dbc.Tabs(
                [new_poi_tab(x, y, map_options), add_poi_tab(x, y)],
                active_tab="create_poi",
            ),
        ],
        style={"height": "90vh"},
        id="cardright",
    ...
    return dbc.Tab(
        [
            dbc.CardBody(
                [
                    html.H4(
                        f"[{x},{y}]", className="card-title", id="point-add-poi-form"
                    ),
                    html.P(
                    ...
                    dcc.Dropdown(
                        options,
                        list(options.keys())[0],
                        id="dropdown-add-poi-form",
                    ),
                    html.Div(
                        [],
                        id="preview-div-add-poi-form",
                        style={"overflow-y": "scroll", "maxHeight": "50vh"},
                    ),
                    html.Br(),
                    dbc.Button(
                        "Add Point of Interest",
                        color="primary",
                        id="button-add-poi-form",
                    ),
                    dcc.Store(id="store-add-poi-form", data=dict()),
                ],
            )
        ],
        label="Add POI",
            ...
            dbc.CardBody(
                [
                    ...
                    dcc.Upload(
                        ...
                        children=html.Div([html.A("Select Thumbnail")]),
                        style={
                            "width": "100%",
                            "height": "60px",
                            "lineHeight": "60px",
                    ...
                    dbc.InputGroup(
                        [
                            dbc.InputGroupText("Map for POI"),
                            dcc.Dropdown(
                                options=map_options, id="map-dropdown-poi-form"
                            ),
                        ],
                        className="mb-3",
                    ),
                    ...
                    dbc.Button(
                        ...
                        color="primary",
                        id="button-create-poi",
                    ),
                    dcc.Store(id="store-poi-form", data=dict()),
                ],
                ...
                + (
                    [
                        ...
                        dbc.Button(
                            "Go to map of POI",
                            color="primary",
                            href=f"{reverse('dash_map')}?id={poi.poi_map.pk}",
                            target="_blank",
                            external_link=True,
                            style={"flex": "0 0", "overflow-y": "visible"},
                        ),
